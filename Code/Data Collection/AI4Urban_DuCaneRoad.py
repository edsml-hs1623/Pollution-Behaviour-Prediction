...
def create_zigzag_pattern(map, start_row, end_row, start_col, end_col, value, slope_row=1, slope_col=1, thickness=2, device='cpu'):
    row = start_row
    col = start_col
        ...
        for i in range(-thickness, thickness + 1):
            for j in range(-thickness, thickness + 1):
                if (0 <= row + i < map.shape[3]) and (0 <= col + j < map.shape[4]):
                    map[0, 0, 1, row + i, col + j] = torch.tensor(value, dtype=torch.float32, device=device)
        row += slope_row
        col += slope_col
...
with torch.no_grad():
    for itime in range(1,ntime+1):
        ...
        # Constant pollution sources
        values_p[0, 0, 1, 75:125, 400:450] = torch.tensor(0.5, dtype=torch.float32, device=device)
        values_p = create_zigzag_pattern(values_p, start_row=375, end_row=425, start_col=150, end_col=200, value=0.5, slope_row=1, slope_col=1, thickness=10, device=device)
...
# output   
        ...
        if save_fig == True and itime % n_out == 0:
            np.save("../Data Preprocessing/Du Cane Road/w"+str(itime), arr=values_w.cpu().detach().numpy()[0,0,:,:])
            np.save("../Data Preprocessing/Du Cane Road/v"+str(itime), arr=values_v.cpu().detach().numpy()[0,0,:,:])
            np.save("../Data Preprocessing/Du Cane Road/u"+str(itime), arr=values_u.cpu().detach().numpy()[0,0,:,:])
            np.save("../Data Preprocessing/Du Cane Road/p"+str(itime), arr=values_p.cpu().detach().numpy()[0,0,:,:])

            plt.imshow(values_w[0,0,1,:,:].cpu())
            plt.colorbar()
            plt.savefig(os.path.join("../Data Preprocessing/Du Cane Road/w", "w"+str(itime)))
            plt.close()

            plt.imshow(values_u[0,0,1,:,:].cpu())
            plt.colorbar()
            plt.savefig(os.path.join("../Data Preprocessing/Du Cane Road/u", "u"+str(itime)))
            plt.close()

            plt.imshow(values_v[0,0,1,:,:].cpu())
            plt.colorbar()
            plt.savefig(os.path.join("../Data Preprocessing/Du Cane Road/v", "v"+str(itime)))

            plt.imshow(values_p[0,0,1,:,:].cpu())
            plt.colorbar()
            plt.savefig(os.path.join("../Data Preprocessing/Du Cane Road/p", "p"+str(itime)))
            plt.close()

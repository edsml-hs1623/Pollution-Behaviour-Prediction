...
with torch.no_grad():
    for itime in range(1,ntime+1):
        values_p[0, 0, 1, 75:125, 400:450] = torch.tensor(5e07, dtype=torch.float32, device=device)
...
# output   
        ...
        if save_fig == True and itime % n_out == 0:
            np.save("../Data Preprocessing/South Kensington/w"+str(itime), arr=values_w.cpu().detach().numpy()[0,0,:,:])
            np.save("../Data Preprocessing/South Kensington/v"+str(itime), arr=values_v.cpu().detach().numpy()[0,0,:,:])
            np.save("../Data Preprocessing/South Kensington/u"+str(itime), arr=values_u.cpu().detach().numpy()[0,0,:,:])
            np.save("../Data Preprocessing/South Kensington/p"+str(itime), arr=values_p.cpu().detach().numpy()[0,0,:,:])

            plt.imshow(values_w[0,0,1,:,:].cpu())
            plt.colorbar()
            plt.savefig(os.path.join("../Data Preprocessing/South Kensington/w", "w"+str(itime)))
            plt.close()

            plt.imshow(values_u[0,0,1,:,:].cpu())
            plt.colorbar()
            plt.savefig(os.path.join("../Data Preprocessing/South Kensington/u", "u"+str(itime)))
            plt.close()

            plt.imshow(values_v[0,0,1,:,:].cpu())
            plt.colorbar()
            plt.savefig(os.path.join("../Data Preprocessing/South Kensington/v", "v"+str(itime)))
            plt.close()

            plt.imshow(values_p[0,0,1,:,:].cpu())
            plt.colorbar()
            plt.savefig(os.path.join("../Data Preprocessing/South Kensington/p", "p"+str(itime)))
            plt.close()
